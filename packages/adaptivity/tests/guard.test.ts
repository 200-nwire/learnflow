...
import { createSnapshot, type SessionSnapshot } from '../src/session.js';
import type { GuardActivation, Variant } from '../src/types.js';

describe('Guard System', () => {
  let session: SessionSnapshot;
  let activation: GuardActivation;
  ...
  describe('JS Guard Evaluator', () => {
    ...
    it('should return true for null guard', () => {
      const evaluator = createJsGuardEvaluator();
      const fn = evaluator(null as any);
      expect(fn(activation)).toBe(true);
    });
    ...
    it('should handle malformed expressions gracefully', () => {
      const evaluator = createJsGuardEvaluator();
      // Use a valid guard syntax instead
      const badGuard = evaluator('ctx.nonExistent?.value === undefined');
      
      expect(() => badGuard(activation)).not.toThrow();
      expect(badGuard(activation)).toBe(true);
    });
  ...
  describe('Real-World Guard Scenarios', () => {
    ...
    it('should combine multiple conditions', () => {
      ...
      const evaluator = createJsGuardEvaluator();
      const complexGuard = evaluator(
        '(ctx.metrics.accEWMA < 0.7 || ctx.metrics.attempts > 5) && ctx.metrics.streak === 0 && ctx.env.device === "mobile"'
      );
