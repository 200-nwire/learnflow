...
describe('Integration Tests - End-to-End Scenarios', () => {
  ...
  describe('Complete Learning Journey', () => {
    it('should adapt through a full lesson with performance tracking', () => {
      // Initialize learner
      const session: SessionSnapshot = createSnapshot({
        ids: {
          userId: 'student_123',
          courseId: 'math_101',
          lessonId: 'fractions',
      ...
      const introSignal = signalFactory.createVariantSelectedSignal(
        session,
        intro,
        introSlot.variants.map(v => ({
          variantId: v.id,
  ...
  describe('Device-Aware Adaptation', () => {
    it('should adapt content for mobile learner', () => {
      ...
      // Desktop guard might not be evaluated if mobile is selected first
      if (result.why.guards['desktop_simulation'] !== undefined) {
  ...
  describe('Complete Signal Workflow', () => {
    it('should track full learning session with signals', () => {
      ...
      const selectionSignal = factory.createVariantSelectedSignal(
        session,
        selection,
        [{ variantId: 'visual_tour', score: 1.0, guardPassed: true }]
      );
  ...
  describe('Performance Deterioration Recovery', () => {
    it('should adapt when student performance drops', () => {
      ...
      // Initially doing well - should get standard (acc is 0.85, not >= 0.9 for hard)
      const initial = selectVariant(slot, session, policy);
      expect(initial.variantId).toBe('standard_problems');
